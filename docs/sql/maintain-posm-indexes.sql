...
用途：
1. 创建 POSM.dbo.IndexMaintenanceLog，用于记录每次索引维护结果。
2. 创建 POSM.dbo.usp_MaintainPosmIndexes。
3. 维护 POSM 数据库内所有启用的 clustered / nonclustered rowstore 索引。
4. 不删除索引、不创建业务索引。

执行示例：
-- 只查看候选索引，不执行维护。
EXEC dbo.usp_MaintainPosmIndexes @DryRun = 1;

-- 正式执行维护。
EXEC dbo.usp_MaintainPosmIndexes;
*/
...
CREATE OR ALTER PROCEDURE dbo.usp_MaintainPosmIndexes
    @DryRun BIT = 0,
...
BEGIN
    ...
    IF DB_NAME() <> N'POSM'
        THROW 53000, N'请在 POSM 数据库执行 dbo.usp_MaintainPosmIndexes。', 1;

    IF @MinPageCount < 1
        THROW 53001, N'@MinPageCount 必须大于 0。', 1;

    IF @ReorganizeThreshold < 0 OR @RebuildThreshold <= @ReorganizeThreshold
        THROW 53002, N'碎片率阈值无效，要求 0 <= reorganize < rebuild。', 1;

    IF @MaxDop < 1
        THROW 53003, N'@MaxDop 必须大于 0。', 1;
